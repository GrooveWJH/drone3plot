...
const logTiming = (fileName: string, phase: string, duration?: number) => {
	...
	lastLogAt = now;
	const durationText =
		typeof duration === "number" ? ` ${duration.toFixed(7)}ms` : "";
	console.log(
...
export const usePointCloudLoader = ({
		...
		async (file: File) => {
			...
			try {
				...
				if (!controller.signal.aborted) {
					setError(
						err instanceof Error ? err.message : "Failed to load LAS file.",
					);
				}
			} finally {
	...
	const pointsColor = useMemo(() => {
		const hasChunkColors = pointCloudChunks.some(
			(chunk) => chunk.colors && chunk.colors.length > 0,
		);
