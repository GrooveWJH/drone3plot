...
export function initLiveStream() {
  ...
  const qualityRadios = Array.from(document.querySelectorAll('input[name="live_quality"]'));
  ...
  const streamCheck = { intervalMs: 1500, timeoutMs: 15000, requestTimeoutMs: 2000 };
  ...
  const waitForStream = async (url) => {
    ...
    while (Date.now() < deadline) {
      ...
      await new Promise((resolve) => setTimeout(resolve, streamCheck.intervalMs));
    }
    return false;
  };
  ...
  startButton.addEventListener('click', () => {
    ...
    postJSON('/api/stream/start', { rtmp_url: rtmpUrl || playbackUrl, video_quality: selectedQuality })
      .then(() => {
        setStageActive(true);
        setPill('live', '直播中');
        setButtons('live');
