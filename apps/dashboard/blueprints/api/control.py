...
def control_auth_request():
    ...
    if not registry.drc:
        return jsonify(
            {"error": "DRC control is only available when connected to a real drone."}
        ), 400
    ...
    if status.get("state") == "error":
        return jsonify(
            {"error": status.get("last_error", "request_control failed")}
        ), 500
...
def control_auth_confirm():
    ...
    if status.get("state") == "error":
        return jsonify(
            {"error": status.get("last_error", "confirm_control failed")}
        ), 500
    return jsonify(status)
