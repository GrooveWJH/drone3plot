...
export function initControlAuth() {
  ...
  const updateBattery = () => {
    getJSON('/api/telemetry')
      .then((payload) => {
        ...
        if (batteryStatus) {
          batteryStatus.textContent = typeof percent === 'number' ? `${percent}%` : '--';
        }
        if (batteryFill) {
  ...
  const handleState = (state, errorText) => {
    ...
    setPill(state, state === 'drc_ready' ? '已连接' : state === 'waiting_for_user' ? '等待确认' : '未连接');
  ...
  getJSON('/api/control/auth/status')
    .then((payload) => {
      if (payload?.error) {
        handleState('unavailable', payload.error);
        return;
      }
      handleState(payload?.state || 'disconnected');
    })
    .catch(() => {
  ...
  requestButton.addEventListener('click', () => {
    ...
    const configPayload = readConfigPayload();
    const applyConfig = configPayload ? postJSON('/api/config', configPayload) : Promise.resolve(null);
    applyConfig
      .catch(() => null)
      .then(() => postJSON('/api/control/auth/request', {}))
      .then((payload) => {
        handleState(payload?.state || 'waiting_for_user');
  ...
  confirmButton.addEventListener('click', () => {
    ...
    postJSON('/api/control/auth/confirm', {})
      .then((payload) => {
        handleState(payload?.state || 'drc_ready');
