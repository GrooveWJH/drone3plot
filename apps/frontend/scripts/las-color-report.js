...
const analyzeLasColors = async (
	filePath,
	sampleTarget = DEFAULT_SAMPLE_TARGET,
) => {
	...
	const pointsPerChunk = Math.max(
		1,
		Math.floor(chunkBytes / header.recordLength),
	);
	...
	try {
		for (
			...
			const byteOffset =
				header.offsetToPointData + pointIndex * header.recordLength;
	...
	return {
		...
		max: stats.max,
		mean:
			stats.samples > 0 ? stats.sum.map((v) => v / stats.samples) : [0, 0, 0],
		zeroRatio,
		maxValue,
		suggestedScale,
...
const printReport = async (filePath) => {
	...
	if (result.firstColors.length > 0) {
		console.log(
			`  前 10 个颜色: ${result.firstColors.map((item) => item.join(",")).join(" | ")}`,
		);
	}
};
