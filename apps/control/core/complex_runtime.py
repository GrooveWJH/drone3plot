...
from apps.control.core.complex_utils import (
    parse_waypoint,
...
class ComplexContext:
    ...
    move_target_index: int | None = None
...
def init_context(
    cfg: Any, position: tuple[float, float, float], current_yaw: float
) -> ComplexContext:
    if cfg.PLANE_USE_RANDOM_WAYPOINTS:
        return ComplexContext(
            current_waypoint=(0.0, 0.0),
            current_target_yaw=generate_random_angle(
                current_yaw, cfg.RANDOM_ANGLE_MIN_DIFF
            ),
...
def init_phase(
    ...
    initial_distance = math.hypot(
        ctx.current_waypoint[0] - current_x, ctx.current_waypoint[1] - current_y
    )
    if (
        initial_distance <= cfg.TOLERANCE_XY
        and abs(current_z - ctx.current_target_z) <= cfg.VERTICAL_TOLERANCE
    ):
        state.phase = "task"
        ...
        ctx.move_target_index = ctx.waypoint_index
...
def _ensure_next_move_target(
    cfg: Any, ctx: ComplexContext
) -> tuple[str, tuple[float, float]]:
    if ctx.move_target_waypoint is None:
        if cfg.PLANE_USE_RANDOM_WAYPOINTS:
            next_index = ctx.waypoint_index + 1
            (
                ctx.move_target_waypoint,
                ctx.move_target_z,
                ctx.move_target_yaw,
                next_desc,
            ) = build_move_target_random(
            ...
            ctx.move_target_index = next_index
        else:
            next_index = ctx.waypoint_index + 1
            (
                ctx.move_target_waypoint,
            ...
            ctx.move_target_index = next_index % len(cfg.WAYPOINTS)
        ctx.move_yaw = yaw_from_points(
            ctx.current_waypoint[0],
            ctx.current_waypoint[1],
...
def _is_last_waypoint(ctx: ComplexContext) -> bool:
    return ctx.total_waypoints is not None and ctx.waypoint_index >= (
    ...
    *,
    ...
    if state.phase == "task":
        ...
        if not task_required:
            ...
            is_final_task = (
                ctx.final_index_start is not None
                and ctx.waypoint_index >= ctx.final_index_start
            )
            if (
                stable_duration == 0.0
                and not state.task_photo_printed
        ...
        if stable_duration is not None:
            ...
            if brake_cooldown_ok and stable_duration >= cfg.PLANE_ARRIVAL_STABLE_TIME:
                ...
                ctx.current_target_z = (
                    ctx.move_target_z
                    if ctx.move_target_z is not None
                    else ctx.current_target_z
                )
                if (
                    ctx.move_target_index is not None
                    and ctx.move_target_index != ctx.waypoint_index
                ):
                    ctx.waypoint_index = ctx.move_target_index
                state.phase = "vertical"
        ...
        if (
            stable_duration is not None
            and stable_duration >= cfg.VERTICAL_ARRIVAL_STABLE_TIME
        ):
            task_required = _is_task_required(ctx)
            state.z_in_tolerance_since = None
            state.task_photo_printed = False
            ...
            is_last_waypoint = (
                ctx.total_waypoints is not None
                and ctx.waypoint_index >= (ctx.total_waypoints - 1)
            )
            if is_last_waypoint:
                ctx.move_target_waypoint = None
                ctx.move_target_index = None
                ctx.move_target_z = None
                ctx.move_target_yaw = None
                ctx.move_yaw = None
                next_desc = f"航点{ctx.waypoint_index}"
            elif cfg.PLANE_USE_RANDOM_WAYPOINTS:
                next_index = ctx.waypoint_index + 1
                (
                    ctx.move_target_waypoint,
                    ctx.move_target_z,
