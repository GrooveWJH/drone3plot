...
export const useTrajectoryState = ({
	...
	const initialTrajectoryId = (() => {
		if (typeof window === "undefined")
			return initialOptions[0]?.id ?? "default";
		const cached = window.localStorage.getItem(TRAJECTORY_ID_STORAGE_KEY);
		if (cached && initialOptions.some((item) => item.id === cached))
			return cached;
	...
	const [trajectoryOptions, setTrajectoryOptions] =
		useState<TrajectoryMeta[]>(initialOptions);
	...
	const refreshTrajectoryOptions = useCallback(() => {
		...
		if (!nextOptions.some((item) => item.id === trajectoryId)) {
			const fallback = findFallbackTrajectoryId(
				TRAJECTORY_SOURCES,
				nextOptions,
			);
