...
def create_plane_yaw_plot(df, title="平面+Yaw控制分析"):
    ...
    if "x_pid_p" in df.columns:
        ...
        fig.add_hline(
            y=0, line_dash="dot", line_color="gray", opacity=0.3, row=4, col=1
        )
    ...
    if "y_pid_p" in df.columns:
        ...
        fig.add_hline(
            y=0, line_dash="dot", line_color="gray", opacity=0.3, row=4, col=2
        )
...
def create_yaw_only_plot(df, title="Yaw角控制分析"):
    ...
    if "yaw_pid_p" in df.columns:
        ...
        fig.add_hline(
            y=0, line_dash="dot", line_color="gray", opacity=0.3, row=3, col=1
        )
...
def create_plane_only_plot(df, title="平面位置控制分析"):
    ...
    if "y_pid_p" in df.columns:
        ...
        fig.add_hline(
            y=0, line_dash="dot", line_color="gray", opacity=0.3, row=3, col=2
        )
...
def print_plane_yaw_statistics(df):
    ...
    print("\n【杆量输出】")
    print(
        f"  Roll杆量: {df['roll_absolute'].min():.0f} ~ {df['roll_absolute'].max():.0f}"
    )
    print(
        f"  Pitch杆量: {df['pitch_absolute'].min():.0f} ~ {df['pitch_absolute'].max():.0f}"
    )
...
def print_plane_only_statistics(df):
    ...
    print("\n【XY位置误差】")
    print(
        f"  X轴平均误差: {error_x_abs.mean():.4f}m ({error_x_abs.mean() * 100:.2f}cm)"
    )
    print(
        f"  Y轴平均误差: {error_y_abs.mean():.4f}m ({error_y_abs.mean() * 100:.2f}cm)"
    )
    print(
        f"  平均距离误差: {df['distance'].mean():.4f}m ({df['distance'].mean() * 100:.2f}cm)"
    )
    print(
        f"  最大距离误差: {df['distance'].max():.4f}m ({df['distance'].max() * 100:.2f}cm)"
    )
    ...
    print(
        f"  Pitch偏移: {df['pitch_offset'].min():.1f} ~ {df['pitch_offset'].max():.1f}"
    )
...
def print_vertical_statistics(df):
    ...
    print(
        f"  偏移范围: {df['throttle_offset'].min():.1f} ~ {df['throttle_offset'].max():.1f}"
    )
...
def main():
    if len(sys.argv) < 2:
        ...
        print(
            "  python -m apps.control.io.visualize data/plane/20240315_143022    # 平面单独数据"
        )
        print(
            "  python -m apps.control.io.visualize data/vertical/20240315_143022 # 垂直高度数据"
        )
        sys.exit(1)
    ...
    if data_type == "plane_yaw":
        ...
        # 创建图表
        print("\n正在生成平面+Yaw控制图表...")
        fig = create_plane_yaw_plot(
            df, title=f"平面+Yaw控制分析 - {os.path.basename(log_dir)}"
        )
        ...
        # 创建图表
        print("\n正在生成Yaw角控制图表...")
        fig = create_yaw_only_plot(
            df, title=f"Yaw角控制分析 - {os.path.basename(log_dir)}"
        )
        ...
        # 创建图表
        print("\n正在生成平面位置控制图表...")
        fig = create_plane_only_plot(
            df, title=f"平面位置控制分析 - {os.path.basename(log_dir)}"
        )
    elif data_type == "vertical":
        print_vertical_statistics(df)
        print_common_statistics(df)
        print("\n正在生成垂直高度控制图表...")
        fig = create_vertical_plot(
            df, title=f"垂直高度控制分析 - {os.path.basename(log_dir)}"
        )
    else:
