...
from pydjimqtt.services.drc_commands import (
    camera_look_at,
    set_camera_zoom,
    take_photo_wait,
)
...
class CameraService:
    ...
    def set_zoom(self, zoom_factor: float, camera_type: str | None = None) -> None:
        ...
        if not payload_index:
            raise RuntimeError(
                "Camera payload index is unknown. Wait for telemetry before sending commands."
            )
    ...
    def select_lens(self, camera_type: str) -> str:
        if camera_type not in self.available_lenses:
            raise ValueError(
                f"Unsupported lens '{camera_type}'. Available: {self.available_lenses}"
            )
        self._current_lens = camera_type
        return self._current_lens

    def look_at(
        self, latitude: float, longitude: float, height: float, locked: bool = False
    ) -> None:
        payload_index = self.client.get_payload_index()
        if not payload_index:
            raise RuntimeError(
    ...
    def take_photo(self, timeout: float = 10.0) -> dict:
        ...
        return take_photo_wait(
            self.client, payload_index=payload_index, timeout=timeout
        )
