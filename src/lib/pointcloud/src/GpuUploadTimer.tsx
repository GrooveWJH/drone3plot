...
const GpuUploadTimer = ({ enabled }: GpuUploadTimerProps) => {
  ...
  const stateRef = useRef<'idle' | 'armed' | 'started' | 'ended' | 'done'>('idle')

  useEffect(() => {
    if (!enabled) return undefined
    const ctx = gl.getContext()
    if (!ctx || typeof (ctx as WebGL2RenderingContext).getExtension !== 'function') {
      console.log('[gpu] WebGL context unavailable')
      return undefined
    }
    const ext = (ctx as WebGL2RenderingContext).getExtension('EXT_disjoint_timer_query_webgl2') as
      | TimerQueryExt
      | null
    extRef.current = ext
    if (!ext) {
  ...
  useFrame(() => {
    if (!enabled || !extRef.current || stateRef.current !== 'ended' || !queryRef.current) return
    const ext = extRef.current
    const ctx = gl.getContext() as WebGL2RenderingContext
    const available = ctx.getQueryParameter(queryRef.current, ctx.QUERY_RESULT_AVAILABLE)
    const disjoint = ctx.getParameter(ext.GPU_DISJOINT_EXT)
    if (available && !disjoint) {
      const timeNs = ctx.getQueryParameter(queryRef.current, ctx.QUERY_RESULT) as number
