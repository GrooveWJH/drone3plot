...
export const loadPcdPointCloud = async (
	...
	const counts = header.counts.length
		? header.counts
		: header.fields.map(() => 1);
	...
	const indexX = header.fields.findIndex(
		(field) => field.toLowerCase() === "x",
	);
	const indexY = header.fields.findIndex(
		(field) => field.toLowerCase() === "y",
	);
	const indexZ = header.fields.findIndex(
		(field) => field.toLowerCase() === "z",
	);
	const indexRGB = header.fields.findIndex(
		(field) => field.toLowerCase() === "rgb" || field.toLowerCase() === "rgba",
	);
	const indexR = header.fields.findIndex(
		(field) => field.toLowerCase() === "r",
	);
	const indexG = header.fields.findIndex(
		(field) => field.toLowerCase() === "g",
	);
	const indexB = header.fields.findIndex(
		(field) => field.toLowerCase() === "b",
	);
	...
	const readFieldMajor = (fieldIndex: number, pointIndex: number) => {
		...
		const fieldStride =
			(header.sizes[fieldIndex] ?? 0) * (counts[fieldIndex] ?? 1);
	...
	const pickLayout = () => {
		...
		const evaluate = (useFieldMajor: boolean) => {
			...
			for (let i = 0; i < totalPoints && count < maxSamples; i += step) {
				...
				const xOffset = useFieldMajor
					? readFieldMajor(indexX, i)
					: base + offsets[indexX];
				const yOffset = useFieldMajor
					? readFieldMajor(indexY, i)
					: base + offsets[indexY];
				const zOffset = useFieldMajor
					? readFieldMajor(indexZ, i)
					: base + offsets[indexZ];
				const x = readBinaryValue(
					view,
					xOffset,
					header.types[indexX],
					header.sizes[indexX],
				);
				const y = readBinaryValue(
					view,
					yOffset,
					header.types[indexY],
					header.sizes[indexY],
				);
				const z = readBinaryValue(
					view,
					zOffset,
					header.types[indexZ],
					header.sizes[indexZ],
	...
	for (
		let i = 0;
		i < totalPoints && accepted < targetPoints;
		i += sampleEvery
	) {
		...
		const xOffset = dataIsFieldMajor
			? readFieldMajor(indexX, i)
			: base + offsets[indexX];
		const yOffset = dataIsFieldMajor
			? readFieldMajor(indexY, i)
			: base + offsets[indexY];
		const zOffset = dataIsFieldMajor
			? readFieldMajor(indexZ, i)
			: base + offsets[indexZ];
		const x = readBinaryValue(
			view,
		...
		if (indexRGB >= 0) {
			const rgbOffset = dataIsFieldMajor
				? readFieldMajor(indexRGB, i)
				: base + offsets[indexRGB];
			...
			const rOffset = dataIsFieldMajor
				? readFieldMajor(indexR, i)
				: base + offsets[indexR];
			const gOffset = dataIsFieldMajor
				? readFieldMajor(indexG, i)
				: base + offsets[indexG];
			const bOffset = dataIsFieldMajor
				? readFieldMajor(indexB, i)
				: base + offsets[indexB];
			r = Number(
				readBinaryValue(
					view,
